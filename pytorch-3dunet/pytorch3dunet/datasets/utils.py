...
def get_train_loaders_1(config):
    ...
    train_datasets = lung.LungDataset(root_dir=loaders_config['root_dir'], transforms=mn_train_tfs, train=True)
    val_datasets = lung.LungDataset(root_dir=loaders_config['root_dir'], transforms=mn_val_tfs, train=False)
    num_workers = loaders_config.get('num_workers', 1)
    logger.info(f'Number of workers for train/val dataloader: {num_workers}')
    batch_size = loaders_config.get('batch_size', 1)
    logger.info(f'Batch size for train/val loader: {batch_size}')
    # when training with volumetric data use batch_size of 1 due to GPU memory constraints
    return {
        'train': DataLoader(dataset=train_datasets, batch_size=batch_size, shuffle=True,
                            num_workers=num_workers),
        'val': DataLoader(dataset=val_datasets, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    }
